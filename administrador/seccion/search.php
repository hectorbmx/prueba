...
<?php  
 ...
 #query para buscqueda de productos del archivo prueba2prueba.php
 if(isset($_POST["query5"]))  
 {  
     
    $output = '';  
    $sql=$conexion->prepare("SELECT * FROM costos_obra WHERE codigo LIKE '%".$_POST["query5"]."%'" ); 
    $sql->execute();
    $result =$sql->fetchall(PDO::FETCH_ASSOC); 
    $output = '<ul class="list-unstyled">';  
    if(($result) > 0)
    ...
    {  
         ...
         foreach 
          ($result as $row)
                  ...
                  {  
                          $output .= '<li>'.$row["codigo"].'</li>';
                          $output .= '<li type="hidden" >'.$row["concepto"].'</li>';  
                  }  
    }  
    else  
    {  
